...
replace nested \kw{for}-loops with efficient relational joins) and demand
...
\footnotetext{Here is a simple system along these lines: introduce a monad $\bless$ and type constructor $\holy$ with the following methods:
...
\todo{either reinvent datalog optimizations (query planning, demand transform) or compile to datalog (query normalization or defunctionalization).}
...
There is a deep tradeoff in programming language design between \emph{expressiveness} and \emph{tractability:} the more powerful the language, the harder it is to reason about programs in that language.
%
...
\todo{Or maybe just having two languages that communicate is enough, \`a la \textsc{Flix}. The best example we have of deep integration is regexes; need more examples justifying this!}
...
\todo{And more generally that by focusing on denotational semantics we can bridge the gap between logic programming and functional programming}
