...
%
Recall from \cref{function-zero-changes-are-derivatives} that a zero change to a function \emph{is} a derivative: the problematic gap between derivatives and changes disappears if the function \emph{does not change}.
%
We ensure this by boxing the argument of \prim{fix}, giving it the type $\isofixLtoL$; as we saw in \cref{why-is-fix-discrete}, at $\iso A$ represents values which do not change.
%
