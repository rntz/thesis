...
that $\atom{grandparent}{\datum{alice},X}$ is derivable. In general we allow
...
yes-or-no question: is the query derivable or not?

%% Although in principle the result of a Datalog program is the set of \emph{all}
%% derivable facts, in practice we use

Observe that ``the set of all derivable facts'' fits neatly into our informal
...
\begin{datalog}
  \atom{underivable}{} \gets \neg \atom{underivable}{}.
\end{datalog}

\noindent
In classical logic, an implication $B \gets A$ is equivalent to $B \vee \neg A$.
Applying this, the above is equivalent to $\atom{underivable}{} \vee
\neg\neg\atom{underivable}{}$, and thus simply to $\atom{underivable}{}$.
%
...
not derivable from the given rules.%
%
...
words, in Datalog \emph{truth} is identified with \emph{derivability}.

Following this principle, we regard anything not derivable as false. This
...
The principle of regarding anything not derivable as false is known as
...
conclusion because it is not derivable.
%
...
%
\todolater{provide some citations for semmle's use cases~\citep{semmle-nasdaq}. Use footnotes with links instead of putting it in the bibliography.}
%
...
and QSQR\todo{citations for magic sets, tabling, QSQR} exist to address this
...
\noindent
This could quickly get out of hand \fixme{informal}{if you want} to do this more
