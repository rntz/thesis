...
\item We make use of set comprehensions, which can be desugared into the monadic
  operator \kw{for} in the usual way~\cite{wadler-monad-comprehensions}.
...
\begin{code}
  ...
  \mem \<\pboxvar x \<\mvar s = \efor{y}{\mvar s} \eeq {\dvar x} {\dvar y}
\end{code}
...
\begin{code}
  ...
  \mvar s \cap \mvar t = \efor x {\mvar s}
  \ewhen{\kern -.5pt \mem \<\eboxvar x \<\mvar t}
  \eset{\dvar x}
\end{code}
...
\renewcommand\bone{\dvar{b_{1}}}
\renewcommand\btwo{\dvar{b_{2}}}
\renewcommand\yone{\dvar{y_{1}}}
\renewcommand\ytwo{\dvar{y_{2}}}
...
\begin{code}
  ...
  \inj 1 \ptuple{} \caseto \etrue\textsf\:;\;\inj 2 \ptuple{} \caseto \efalse
...
\begin{code}
  ...
  \efor{\kernj}{\mvar r_1\<\eboxtuple{\dvar{s}, \dvar{i}}}
...
\begin{code}
  ...
  = \efixis{x}{\bigl(\eset{\dvar{i}} \vee
    \efor {\kernj} {\mvar x} \dvar r\<\eboxtuple{\dvar{s}, \dvar j}
    \bigr)}
\end{code}
...
%\renewcommand\ctor[1]{\textsf{\scshape\MakeUppercase{\lsstyle#1}}}
...
\begin{code}
  ...
  ~|~ \ctor{add} ~|~ \ctor{sub} ~|~ \ctor{mul} ~|~ \ctor{div}\\
  \kw{data}~\typename{atom} = \ctor{var} \<\typename{var} ~|~ \ctor{num} \<\tint\\
  \kw{data}~\typename{expr} =
    \ctor{atom} \<\typename{atom} ~|~
    \ctor{apply} \<\typename{op} \<\typename{atom} \<\typename{atom}\\
  \kw{data}~\name{stmt} =
