...
\item We mentioned earlier that Datafun's boolean type \tbool\ is ordered
  ...
  tuples, $\tset 1$, with \etrue\ as the singleton $\esetraw{\etuple{}}$ and
  \efalse\ as the empty set $\esetraw{}$. In a loop over a boolean, $\eforvar x
  e f$, the variable $\dvar x$ has no useful information, so for concision we
  abbreviate these to a ``one-sided'' conditional, $\efor{e} f$:

  \begin{equation*}
    \efor{e} f = 
    \begin{cases}
      f & \text{if }e\text{ is true}\\
      \bot & \text{if }e\text{ is false}
    \end{cases}
  \end{equation*}

  Compared with encoding booleans as a sum type \(\tunit + \tunit\) and our
...
\item We make use of set comprehensions, which can be desugared into the monadic
  operator \kw{for} in the usual way~\citep{wadler-monad-comprehensions}. \todo{discuss discrete case-analysis and how we integrate it with comprehensions.}
...
\item We augment our pattern-matching construct \kw{case} to support matching not only tuples and tagged values, but also to allow testing equality. The pattern $\peq{e}$ matches only a value equal to $\eiso{e}$. This is particularly useful when combined with set comprehensions, to implement filters or equijoins.
