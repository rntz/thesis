...
\todo{figure with logical relation}
...
\begin{theorem}[Fundamental property]\label{theorem-seminaive-fundamental}
...
\begin{proof}
  See \cref{proof-fundamental-property}. \todo{prove fundamental property}
\end{proof}
...
\begin{lemma}[Equality changes]\label{lemma-equality-changes}
  If\/ $\weirdat{\eqt A}{\dx}{x}{a}{y}{b}$ then\/ $x = a$ and\/ $y = b$.
\end{lemma}

\begin{lemma}[Dummy is zero at eqtypes]\label{lemma-lr-dummy}
  If\/ $x \in \den{\eqt A}$ then $\weirdat{\eqt A}{\dummy\<x}{x}{x}{x}{x}$.
\end{lemma}

\begin{proof}
  Induct on $\eqt A$. \todo{check equality changes \& dummy zero proofs}
\end{proof}
...
theorem.
%
\Cref{lemma-lr-dummy} is an analogue of \cref{lemma-dummy-change}, showing that
...
\begin{proof}
  \todo{sketch discrete contexts don't change proof}
\end{proof}
...
\begin{proof}
  \todo{sketch context stripping proof}
\end{proof}
