...
Finally, the rule \rn{fix} for fixed points $\efix f$ \splittodo{rewrite explanation of fix typing rule}{also, for why fix is discrete see \cref{why-is-fix-discrete}, has to do with our strategy for evaluating Datafun efficiently} a recursive expression $e : \fixt{L}$ which refers to its own result as $\mvar x$.
%
...
\todo{make a lemma for this, label lemma-fixed-point} 
\todolater{explain why fix is discrete}
