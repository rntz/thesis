...

\subsection{The finite powerset monad and semilattice objects}

0. We've already seen the finite powerset monad: ...

1. Semilattice objects generalize the finite powerset monad.

2. Semilattice objects are ones where $VL = \D L$ is a semilattice and $\changesat{L}{\dx} x y \iff x \vee \dx = y$. This definition is rather restrictive, but we'll see soon that something along these lines seems necessary in order to let semi\naive\ fixed points workout.

3. Join is straightforward. This leaves us with singleton, isEmpty, collect and fix, which both use the discreteness comonad $\iso$, which we haven't yet defined.


\subsection{Fixed points and the box comonad}

To handle \prim{fix} expressions we need a class of fixed-point morphisms, $\morph{fix}_L : \iso(L \expto L) \to L$, for each ``semilattice object'' $L$. We will defer 
