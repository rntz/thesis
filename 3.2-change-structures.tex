...
Our eventual destination is a static transformation on Datafun source code which implements semi\naive\ evaluation (\cref{section-phi-delta}).
%
This transformation, originally presented by \cite{seminaive-datafun}, predates the construction of \ChangePoset\ presented here and is independent of it.
%
...
%
In \ChangePoset\ we will cover products, sums, exponentials, the discreteness comonad, and fixed points, as they are the most significant for understanding the broad structure of our approach.
%
%% XXX: explain why we don't cover the rest (they are straightforward, or similar, or unimportant)
\todo{cover the other structures briefly}
...
Finally, the terminal map $\fork{}$, projection $\pi_i$, and the tupling $\fork{f,g}$ of $f : A \to B$ and $g : A \to C$ are all the same as in \Poset\ (thus inheriting the necessary universal properties), with derivatives given by:
%
...
%% \begin{align*}
%%   \fork{}' &: \iso VA \to \Delta A \to \terminalobject
%%   &
%%   &\fork{}' \<a \<\da = \tuple{}
%%   \\
%%   \pi_i' &: \iso(VA_1 \x VA_2) \to \D A_1 \x \D A_2 \to \D A_i
%%   &
%%   &\pi_i' \<(x_1,x_2) \<(\dx_1,\dx_2) = \dx_i
%%   \\
%%   \fork{f,g}' &: \iso VA \to \D A \to \D B \x \D C
%%   &
%%   &\fork{f,g}' \<a \<\da = \tuple{f'\<a\<\da,\, g'\<a\<\da}
%% \end{align*}
...

\subsection{Sums}\label{change-structure-for-sums}
...
\newcommand\emptykrof{\krof{\thinspace}}

The initial map $\emptykrof$, injection $\injc_i$, and case-analysis $\krof{f_1,f_2}$ (given $f_1 : A_1 \to C$, $f_2 : A_2 \to C$) are the same as in \Poset\ (inheriting its universal properties), with derivatives as follows:

\begin{align*}
  \emptykrof &: \initialobject \to A
  &
  \emptykrof' &= \emptykrof \quad\text{(the domain is empty)}
  \\
...
\noindent
Correctness of $\emptykrof'$ is vacuous; correctness of $\injc_i'$ follows directly from \rn{sum change}; but the definition of $\krof{f_1,f_2}'$ requires explanation.
%
...
\noindent\label{function-zero-changes-are-derivatives}%
...
Instead, we deliberately limit the scope of our approach to avoid the need to incrementally maintain fixed points. As we mentioned in \cref{fix-is-discrete}, in Datafun \prim{fix} is not treated as a monotone operator; correspondingly the morphisms we require to interpret it are not $\morph{fix} : (L \expto L) \to L$ but rather $\morph{fix} : {\color{IsoRed} \iso}(L \expto L) \to L$. The idea here is that, just as $\iso$ in \Poset\ captures \emph{non-monotonicity} in an otherwise monotone world, in \ChangePoset\ we can use it to capture \emph{non-differentiability} or \emph{non-incrementalizability} in an otherwise differentiable world.
...
\begin{align*}
  ...
  &
  \varepsilon_A' \<x \<\tuple{} &= \zero_x\quad\textsf{(see footnote\footnotemark)}
  \\
...
\footnotetext{It is again important here that in \ChangePoset\ our morphisms are merely differentiable, not equipped with derivatives, that is, that we do not distinguish morphisms by their derivative. Otherwise naturality of $\varepsilon$ would require that $(\varepsilon \compose \iso f)' = (f \compose \varepsilon)'$, which requires $\varepsilon'\<(f\<x)\<() = f'\<x\<(\varepsilon'\<x\<())$ and thus $\zero_{(f\<x)} = f'\<x\<\zero_x$, which is difficult to guarantee without unique zero-changes.}
