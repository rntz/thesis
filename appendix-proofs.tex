...
in the text. \todolater{But maybe it should be?} \fixme{jeremy}{Is dividing the appendix into sections according to your chapters the right thing to do?}
...
%
  ...
  \item[Case $\infer{ }{\J \bot \G {\eqt L}}$.]
    Recall that $\phi \bot = \bot$ and $\delta \bot = \bot$ and $\den{\bot}\<\sigma = \bot$. Thus it STS $\weirdat{\eqt L} \bot \bot \bot \bot \bot$, which holds by \cref{lemma-semilattice-changes}.

  \item[Case $\infer{(\J{e_i} \G {\eqt L})_i}{\J{e_1 \vee e_2} \G {\eqt L}}$.]
...
%
  ...
  \includepdf[pages=2-14]{remarkable-seminaive-proof}
\end{proof}
