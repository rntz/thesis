...
%to speed up $\efixis x e$ we don't want the change to $\efn x e$; we want its derivative.
%
...
{\bfseries\boldmath decorate expressions of type ${\iso A}$ with their zero
  changes.}
%
This makes derivatives available exactly where we need them: at \kw{fix}
expressions.
...
e {\hd x A, \hm y B} C$. Then \cref{thm:type-correct} tells us:
...
%%   \sqsubseteq \stripcx{\iso\Phi\G, \DP\G}$.}
...
\splittodo{explain}{ that $\phi$, $\delta(\esplit e)$ are effectively book-keeping
...
\begin{proof}
  \todo{prove dummy is 0 on eqtypes.}
\end{proof}
