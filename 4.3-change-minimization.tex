...
\todo{rewrite to explain how change minimization is implemented}
...
\footnotetext{On finite sets \(\dx \setminus x\) is monotone in \(\dx\) but not \(x\), and this pattern will hold for our other semilattice types as well, but monotonicity in either argument is not required for correctness.}
